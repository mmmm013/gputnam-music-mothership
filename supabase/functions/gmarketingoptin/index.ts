...
Deno.serve(async (req: Request) => {
  ...
  try {
    const { email, metadata } = await req.json().catch(() => ({}));
    ...
    const resp = await fetch(`${supabaseUrl}/rest/v1/rpc/g_marketing_optin_upsert`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
    ...
    return new Response(
      JSON.stringify({ ok: true, status: "opted_in" }),
      { status: 200, headers }
    );
  } catch (e) {
