...
export default function Hero() {
  ...
  return (
    // BRANDING: AMBER (#FFCA28) & WHEAT (#F5DEB3)
    <section className="relative min-h-screen w-full bg-[#FFCA28] text-[#3E2723] pt-24 flex flex-col items-center overflow-hidden font-sans">
      ...
      {/* NAVIGATION: WHEAT BACKGROUND */}
      <nav className="absolute top-0 w-full p-6 flex justify-between items-center bg-[#F5DEB3]/90 backdrop-blur-sm z-50 border-b border-[#3E2723]/10 shadow-sm">
        <div className="flex gap-6 text-xs font-bold tracking-[0.2em] uppercase text-[#3E2723]">
          <span className="cursor-pointer hover:text-[#D2691E] transition">GPM Home</span>
          <span className="cursor-pointer hover:text-[#D2691E] transition">Artists</span>
          <span className="cursor-pointer hover:text-[#D2691E] transition">K-Series</span>
          <span className="cursor-pointer hover:text-[#D2691E] transition flex items-center gap-1"><Star size={10}/> Heroes</span>
        </div>
      ...
      <div className="container mx-auto px-4 text-center z-10 mt-8">
        <h1 className="text-6xl md:text-8xl font-black tracking-tighter text-[#3E2723] mb-2 drop-shadow-sm">
          G Putnam <br/> Music
        </h1>
        ...
        <p className="text-[#3E2723] text-lg max-w-xl mx-auto mb-12 font-bold">
          The GPM Mothership. Find your feeling.
        </p>

        {/* SEARCH BAR: WHEAT FIELD */}
        <div className="max-w-xl mx-auto relative mb-16 group">
          <input 
            type="text" 
            placeholder="How are you feeling? (e.g. 'Heroes', 'Love')"
            className="w-full bg-[#F5DEB3] border border-[#3E2723] placeholder-[#3E2723]/50 text-[#3E2723] font-bold text-xl px-8 py-6 rounded-2xl shadow-xl focus:outline-none focus:ring-2 focus:ring-[#3E2723] transition-all"
          />
          <button className="absolute right-4 top-1/2 -translate-y-1/2 p-3 bg-[#3E2723] rounded-xl text-[#F5DEB3] hover:scale-105 transition">
            <Search size={24} />
          </button>
        </div>
      </div>
      ...
      <div className="container mx-auto px-4 pb-20 grid md:grid-cols-2 gap-8 items-start max-w-6xl z-10">
        ...
        {/* LEFT: TRACK LIST (WHEAT BACKGROUND) */}
        <div className="bg-[#F5DEB3] border border-[#3E2723] rounded-xl shadow-2xl overflow-hidden h-[400px] flex flex-col">
          <div className="p-6 border-b border-[#3E2723]/20 bg-[#FFE082]">
            <div className="flex justify-between items-center">
              <div>
                <span className="text-xs font-bold text-[#3E2723]/70 uppercase tracking-widest">Wounded & Willing Collection</span>
                <h3 className="text-xl font-bold text-[#3E2723]">{currentFP.title}</h3>
              </div>
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-2 scrollbar-hide">
            {loading ? (
              <p className="text-[#3E2723]/50 text-center py-10">Loading GPM Catalog...</p>
            ) : (
              playlist.map((track, idx) => (
                <div 
                  key={track.id || idx}
                  onClick={() => playTrack(idx)}
                  className={`group flex items-center justify-between p-3 rounded-lg cursor-pointer transition ${idx === currentTrackIndex ? 'bg-[#3E2723] text-[#F5DEB3]' : 'bg-[#FFF8E1] text-[#3E2723] hover:bg-white'}`}
                >
                  <div className="flex items-center gap-3 overflow-hidden">
                    <div className="w-8 h-8 flex items-center justify-center bg-black/5 rounded-full">
        ...
        {/* RIGHT: PLAYER VISUAL (AMBER/BROWN) */}
        <div className="bg-[#3E2723] p-8 rounded-3xl shadow-2xl text-center border border-[#F5DEB3]/50 h-[400px] flex flex-col items-center justify-center relative overflow-hidden">
           <div className="w-40 h-40 bg-[#FFCA28] rounded-full flex items-center justify-center mb-6 shadow-xl relative z-10 ring-4 ring-[#F5DEB3]/50">
              <button onClick={togglePlay} className="hover:scale-110 transition">
                {isPlaying ? <Pause size={56} className="text-[#3E2723]" fill="currentColor" /> : <Play size={56} className="text-[#3E2723] ml-2" fill="currentColor" />}
              </button>
           </div>
           
           <div className="relative z-10">
             <h2 className="text-2xl font-black text-white mb-2 max-w-xs mx-auto leading-tight">{cleanTitle(activeTrack)}</h2>
             <p className="text-[#F5DEB3] font-bold uppercase tracking-widest text-xs">G Putnam Music, LLC</p>
           </div>
        </div>
